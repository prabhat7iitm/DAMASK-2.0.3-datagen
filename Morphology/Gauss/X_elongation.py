...
# Grain morphology
sigma_x, sigma_y = 5, 2   # elongation along Y
...
for case_id in tqdm(range(n_cases), desc="Generating cases"):
    ...
    for g in range(n_grains):
        ...
        # Enforce ±5° limit for φ1 and φ2
        delta[0] = np.clip(delta[0], -5.0, 5.0)
        delta[2] = np.clip(delta[2], -5.0, 5.0)
        euler[0] %= 360.0
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(img_dir, f"microstructure_{case_id:04d}.png"),
        dpi=150
    )
    plt.close()
